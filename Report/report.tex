...
output logic [(1+(`N/2)):0][7:0]	out
	...
	\includepdf[scale=0.85,pages=1,pagecommand=\chapter{Project Brief},offset=0 -5cm]{source/Brief.pdf}
...
.2 Approach 1.
.3 Hardware.
.4 Source\DTcomment{Contains all System Verilog source code}.
.4 Simulation\DTcomment{Contains ModelSim project and testbenches}.
.3 Software.
.4 Source\DTcomment{Makefile uses all C files in here}.
.4 build\DTcomment{Make compiles to here}.
.4 make\DTcomment{Makefile}.
.2 Approach 2.
.3 Hardware.
.4 Source\DTcomment{Contains all System Verilog source code}.
.4 Simulation\DTcomment{Contains ModelSim project and testbenches}.
.4 Synthesis\DTcomment{Contains Quartus Prime projects for each mode}.
.3 Software.
.4 Source\DTcomment{Makefile uses all C files in here}.
.4 build\DTcomment{Make compiles to here}.
.4 make\DTcomment{Makefile}.
...
.3 Software.
.4 Source\DTcomment{Makefile uses all C files in here}.
.4 BUILD\DTcomment{Make compiles to here if BUILD is defined}.
.4 DEBUG\DTcomment{Make compiles to here if DEBUG is defined}.
.4 make\DTcomment{Makefile}.
.4 test\DTcomment{Text file to be read}.
}
